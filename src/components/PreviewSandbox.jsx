...
Return ONLY valid JSON (no markdown):
{
  ...
  "extracted": {"key field": "value found"},
  "steps": ["Step 1: what agent does first", "Step 2: etc"],
  "would_produce": "specific description of the output",
  "gaps": ["specific thing missing that would improve this result"],
  "confidence_note": "one sentence on what would most improve accuracy"
      ...
      if (fileB64 && fileType === "application/pdf") {
        messages = [{ role: "user", content: [
          ...
          { type: "text", text: "Process this document according to the agent specification above." }
        ]}];
      } else {
        messages = [{ role: "user", content: "Process this input:\n\n" + (input.trim() || "File: " + fileName) + "\n\nShow exactly what the agent would do." }];
      }
      const raw = await callClaude(messages, system, 1000);
      const parsed = parseJSON(raw);
      if (parsed) { setResult(parsed); } else { setError("Could not parse response. Try again."); }
    } catch (e) { setError("Connection error: " + e.message); }
    setLoading(false);
  };

  const handleFile = async (file) => {
    setFileName(file.name); setFileType(file.type); setInput(""); setResult(null);
    try {
      const content = await readFileAsText(file);
      if (file.type === "application/pdf") { setFileB64(content.split(",")[1]); setInput("[PDF: " + file.name + "]"); }
      else { setFileB64(null); setInput(content.substring(0, 4000)); }
    } catch (e) { setError("Could not read file: " + e.message); }
  };

  const missingSteps = [];
  if (!agentData?.concept) missingSteps.push("Define your agent concept");
  if (!agentData?.trigger && !agentData?.triggers) missingSteps.push("Define what kicks it off");
  if (!agentData?.inputs) missingSteps.push("Define what it reads");
  if (!agentData?.outputs) missingSteps.push("Define what it produces");
