...
const getFallback = (concept, key) => {
  ...
  if (isEmail) {
    const m = {
      trigger: "When I manually upload an email or document file directly into the agent interface.",
      inputs: "Email subject, sender name, full body text, and any attachments — uploaded as a file.",
      outputs: "A draft reply document ready for my review, saved to a shared folder.",
      knowledge: "Past email threads saved as reference files, standard response templates.",
      systems: "A shared Box or Google Drive folder for input and output files.",
      humanGate: "Before I send any reply externally, and when the topic requires judgment it doesn't have context for.",
    };
    return m[key] || "";
  }
  const generic = {
...
const getFallbackHints = (stepKey, concept) => {
  const maps = {
    ...
    outputs: [
      ...
      { gap: "Where does the output go?", options: ["downloaded directly from the agent as a file", "saved to a shared Box or Google Drive folder", "saved to a folder and notification sent"] },
    ],
    systems: [
...
const analyzeTemplate = async (file, setSuggestions, setAnalysis, setAnalyzing) => {
  ...
  try {
    ...
    let messages;
    const prompt = 'Analyze this form that an AI agent will fill out from a source document uploaded by the user. Return JSON only:\n{"fields":["all field names"],"source_document_fields":["fields extracted from uploaded source doc"],"user_provided_fields":["fields user types manually"],"computed_fields":["fields agent calculates"],"required_inputs":"what source document does user upload?","trigger":"when is this form filled out?","outputs":"completed form description","humanGate":"when should human review?","summary":"what is this form for?"}';
...
const getDomainContext = (text) => {
  ...
  if (l.includes("submittal log") || l.includes("submittal register"))
    return "DOMAIN FACT: A submittal log is an OUTPUT built by reading construction SPECIFICATION documents. Input = spec docs uploaded by user. Output = the log. Do not suggest submittals as inputs.";
  if (l.includes("rfi") || (l.includes("change order") && l.includes("construction")))
    return "DOMAIN FACT: RFI/change order agents read RFI documents (uploaded by user) and produce impact analysis memos or notice letters (outputs).";
  if (l.includes("punch list"))
    return "DOMAIN FACT: Punch list agents read inspection notes or photos (uploaded as files) and produce a formatted punch list document (output).";
  ...
  if (l.includes("medical record") || l.includes("patient record") || l.includes("clinical note"))
    return "DOMAIN FACT: Medical record agents read clinical documents (uploaded by user) and produce structured summaries or coded entries (outputs).";
  if (l.includes("lead") && (l.includes("qualify") || l.includes("score") || l.includes("enrich")))
    return "DOMAIN FACT: Lead qualification agents read prospect data files (uploaded by user) and produce scored/enriched lead records (outputs).";
  if (l.includes("proposal") && (l.includes("generat") || l.includes("creat") || l.includes("draft") || l.includes("build")))
    return "DOMAIN FACT: Proposal generation agents read deal data and requirements (uploaded by user) and produce formatted proposal documents (outputs).";
  ...
  if (l.includes("reconcil"))
    return "DOMAIN FACT: Reconciliation agents read two or more data files (uploaded by user) and produce a discrepancy report (output).";
  if (l.includes("resum") || (l.includes("cv") && (l.includes("screen") || l.includes("review") || l.includes("rank"))))
    return "DOMAIN FACT: Resume screening agents read job descriptions and resumes (uploaded by user) and produce ranked shortlists or fit scores (outputs).";
  if (l.includes("onboard"))
    return "DOMAIN FACT: Onboarding agents read new hire data and policy documents (uploaded by user) and produce checklists or welcome communications (outputs).";
  if (l.includes("ticket") && (l.includes("triage") || l.includes("route") || l.includes("classif") || l.includes("priorit")))
    return "DOMAIN FACT: Ticket triage agents read support tickets (uploaded or pasted) and produce classified, prioritized assignments (outputs).";
  if ((l.includes("email") || l.includes("inbox")) && (l.includes("draft") || l.includes("reply") || l.includes("response") || l.includes("triage")))
    return "DOMAIN FACT: Email agents read email files (uploaded by user or pasted — NOT connected to live email) and produce draft reply documents (outputs). No email API required.";
  ...
  if (l.includes("content") && (l.includes("generat") || l.includes("creat") || l.includes("draft") || l.includes("write")))
    return "DOMAIN FACT: Content generation agents read briefs and reference material (uploaded by user) and produce written content (outputs).";
...
const STEPS = [
  {
    ...
    coachQ: (val, concept, ctx, correction) => {
      ...
      return (domainCtx ? domainCtx + "\n\n" : "") + correctionCtx +
        ...
        "CONSTRAINT: This agent has NO email API, NO platform integrations, NO OAuth. User manually uploads files or pastes content. All suggestions must reflect this.\n\n" +
  ...
  },
  {
    ...
    coachQ: (val, concept, ctx) => "What has been defined so far:\n" + (ctx||concept) + "\n\nCross-reference docs: \"" + val + "\"\n\nSimplest options first. All docs uploaded once by user — no live connections. For each gap provide 3 options.\n\nReturn ONLY JSON array:\n[{\"gap\":\"gap description\",\"options\":[\"option A\",\"option B\",\"option C\"]}]",
  },
  {
    key: "knowledge",
    ...
    coachQ: (val, concept, ctx) => "What has been defined so far:\n" + (ctx||concept) + "\n\nReference data: \"" + val + "\"\n\nDistinguish between (1) structured lookup data and (2) formatted few-shot examples. Do NOT suggest prose documents or behavioral rules. For each gap provide 3 short specific options.\n\nReturn ONLY JSON array:\n[{\"gap\":\"gap description\",\"options\":[\"option A\",\"option B\",\"option C\"]}]",
  },
  {
    key: "systems",
    ...
    optional: true,
    coachQ: (val, concept, ctx) => "What has been defined so far:\n" + (ctx||concept) + "\n\nSystems: \"" + val + "\"\n\nCRITICAL: NO OAuth, NO platform APIs. ONLY suggest: (1) files uploaded directly into agent interface, (2) Box shared folder, (3) Google Drive shared folder, (4) downloaded output file. Do NOT suggest email, CRM, project management platforms, or any API.\n\nReturn ONLY JSON array:\n[{\"gap\":\"gap description\",\"options\":[\"option A\",\"option B\",\"option C\"]}]",
  },
  {
    key: "humanGate",
    ...
    starterHints: [
      ...
      { gap: "For any values I always need to confirm", options: ["ask me to confirm key reference values before starting", "use whatever values I provide at the start", "pre-fill from the last run and let me change anything before it proceeds"] },
    ],
    coachQ: (val, concept, ctx) => "What has been defined so far:\n" + (ctx||concept) + "\n\nHuman oversight: \"" + val + "\"\n\nWhat oversight gates are missing? For each gap provide 3 short injectable options.\n\nReturn ONLY JSON array:\n[{\"gap\":\"gap description\",\"options\":[\"option A\",\"option B\",\"option C\"]}]",
  },
  {
    key: "name",
  ...
  return (
    <div style={{ background: addedList.length > 0 ? C.success + "08" : "#0D1B27", border: "1px solid " + (addedList.length > 0 ? C.success + "33" : "#1D3246"), borderRadius: "8px", padding: "0.7rem 0.8rem", marginBottom: "0.4rem" }}>
      <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
        <span style={{ color: addedList.length > 0 ? C.success : C.accent, flexShrink: 0, fontSize: "0.65rem", marginTop: "3px" }}>{addedList.length > 0 ? "+" : "2192"}</span>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "1.1rem 1.5rem" }}>
            ...
            {window._agentAcademyComplete && (
              <button onClick={() => { window._agentAcademyComplete(); window._agentAcademyComplete = null; }}
                ...
                {"START ACADEMY 2014 DEPLOY 0026 IMPROVE 2192"}
              </button>
            )}
